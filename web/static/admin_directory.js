...
let directory = null;
async function loadAndDrawDirectory() {
...
async function loadDirectory() {
    const { json, err } = await ims.fetchNoThrow(url_directory, {
...
function groupTitlesByID(groups) {
    const byID = new Map();
...
function drawPersons() {
    ...
    for (const person of directory?.persons ?? []) {
        const rowFrag = el.personRowTemplate.content.cloneNode(true);
        ...
        const showEditModal = row.querySelector(".show-edit-modal");
        showEditModal.addEventListener("click", (_e) => {
...
function drawMembershipCheckboxes(container, groups, memberOf, kind) {
...
function drawGroupList(list, groups, kind) {
    ...
    for (const group of groups) {
        ...
        const groupID = group.id;
        const liFrag = el.groupLiTemplate.content.cloneNode(true);
        ...
        const activeButton = li.querySelector(".group-active");
        activeButton.addEventListener("click", async (_e) => {
            await sendGroup(kind, { id: groupID, active: false });
            await loadAndDrawDirectory();
        });
        const inactiveButton = li.querySelector(".group-inactive");
        inactiveButton.addEventListener("click", async (_e) => {
            await sendGroup(kind, { id: groupID, active: true });
            await loadAndDrawDirectory();
        });
        const renameButton = li.querySelector(".group-rename");
        renameButton.addEventListener("click", async (_e) => {
            const newTitle = prompt(`Rename ${kind} "${group.title ?? ""}"?\n\n` +
                "Note that event access rules refer to teams and positions by name, " +
                "so renaming will change which rules apply to this group's members.", group.title ?? "");
        ...
        const deleteButton = li.querySelector(".group-delete");
...
// Persons
//
async function createPerson(sender) {
...
function modalPersonID() {
    return ims.parseInt10(el.editPersonModal.dataset["personId"]);
}
async function setPersonHandle(sender) {
...
async function setPersonEmail(sender) {
    const id = modalPersonID();
    if (id == null) {
        return;
    }
...
async function setPersonActive(sender) {
    const id = modalPersonID();
    if (id == null) {
        return;
    }
...
async function setPersonOnsite(sender) {
    const id = modalPersonID();
    if (id == null) {
        return;
    }
...
async function setPersonMemberships(kind) {
    const id = modalPersonID();
    if (id == null) {
        return;
    }
...
async function setPersonPassword(sender) {
    const id = modalPersonID();
...
async function deletePerson(_sender) {
    const id = modalPersonID();
    if (id == null) {
        return;
    }
    if (!confirm("Delete this person outright? Consider deactivating them instead, " +
...
async function sendPersonFromControl(sender, edits) {
...
async function sendPerson(edits) {
...
// Teams and positions
//
async function createTeam(sender) {
...
async function createPosition(sender) {
...
async function sendGroup(kind, edits) {
...
function alertFailure(message, err) {
    const full = `${message}:\n${err}`;
    console.error(full);
    window.alert(full);
}
