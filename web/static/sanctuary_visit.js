...
async function initSanctuaryVisitPage() {
    ...
    if (!ims.eventAccess.readVisits) {
        ims.setErrorMessage(`You're not currently authorized to read Visits in Event "${ims.pathIds.eventName}".`);
...
async function loadAndDisplayVisit() {
    await loadVisit();
    displayVisit();
}
...
async function sendEdits(edits) {
    ...
    if (number == null && resp != null) {
        ...
        let newNumber = resp.headers.get("IMS-Visit-Number");
        ...
        drawVisitTitle("for_display");
        window.history.pushState(null, document.title, `${ims.urlReplace(url_viewVisits)}/${newNumber}`);
...
async function editArrivalTime(selectedDates, _dateStr, sender) {
    const prevDate = visit?.arrival_time ? new Date(visit.arrival_time) : undefined;
...
async function editDepartureTime(selectedDates, _dateStr, sender) {
    const prevDate = visit?.departure_time ? new Date(visit.departure_time) : undefined;
    const newDate = selectedDates[0];
    if (newDate?.getTime() === prevDate?.getTime()) {
        // Either they're the same valid time, or neither is set, so there's nothing to do.
        return;
...
function drawRangers() {
    const rangers = visit?.rangers ?? [];
    rangers.sort((a, b) => (a.handle ?? "").localeCompare(b.handle ?? ""));
    const rangerItemTemplate = document.getElementById("visit_rangers_li_template");
    const rangersElement = document.getElementById("visit_rangers_list");
