...
async function initSanctuaryVisitsPage() {
    ...
    if (!ims.eventAccess.readVisits) {
        ims.setErrorMessage(`You're not currently authorized to view Visits in Event "${ims.pathIds.eventName}".`);
    ...
    visitsTable = new DataTable("#visits_table", {
        ...
        "ajax": function (_data, callback, _settings) {
            async function doAjax() {
                const { json, err } = await ims.fetchNoThrow(ims.urlReplace(url_visits), null);
        ...
        "createdRow": function (row, visit, _index) {
...
//
function initSearch() {
    function modifiedAfter(visit, timestamp) {
    ...
    visitsTable.search.fixed("modification_date", function (_searchStr, _rowData, rowIndex) {
        const visit = visitsTable.data()[rowIndex];
        return !(_showModifiedAfter != null &&
            !modifiedAfter(visit, _showModifiedAfter));
    });
    visitsTable.search.fixed("status", function (_searchStr, _rowData, rowIndex) {
        if (_showStatus === "all") {
            return true;
        }
