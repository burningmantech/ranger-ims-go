...
async function doSearch() {
    ...
    const { resp, json, err } = await ims.fetchNoThrow(`${url_search}?${params.toString()}`, null);
...
function renderResults(hits) {
    ...
    for (const hit of hits) {
        const rowFrag = el.resultRowTemplate.content.cloneNode(true);
        ...
        const link = row.querySelector(".result-number a");
        link.href = resultURL(hit);
        link.textContent = hit.number.toString();
        const created = new Date(hit.created);
        const createdCell = row.querySelector(".result-created");
