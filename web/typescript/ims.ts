...
export function setupMapLink(mapLink: HTMLAnchorElement, events: EventData[]|null): void {
...
export function announce(msg: string): void {
    const region: HTMLElement|null = document.getElementById("aria_live");
    if (!region) {
        return;
    }
...
// changed.
export function newUpdateAnnouncer(noun: string, debounceMs: number = 3000): () => void {
    let count = 0;
...
export function newRemoteUpdateAnnouncer(msg: string, debounceMs: number = 3000): {
    noteLocalEdit: () => void;
    announceUpdate: () => void;
} {
    let lastLocalEdit = 0;
...
export function enableKeyboardSorting(tableId: string): void {
    ...
    function makeSortControlsFocusable(): void {
        for (const order of container!.querySelectorAll<HTMLElement>("thead .dt-column-order")) {
            if (order.tabIndex !== 0) {
                order.tabIndex = 0;
            }
        }
    ...
    container.addEventListener("keydown", function (e: KeyboardEvent): void {
