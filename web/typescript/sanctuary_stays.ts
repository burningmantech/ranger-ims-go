...
let staysTable: ims.DataTablesTable|null = null;
...
async function initSanctuaryStaysPage(): Promise<void> {
    ...
    if (!ims.eventAccess!.readStays) {
        ims.setErrorMessage(
            `You're not currently authorized to view Stays in Event "${ims.pathIds.eventName}".`
        );
...
function initStaysTable() {
    ...
    staysTable!.on("init", function (): void {
...
function initDataTables() {
    ...
    staysTable = new DataTable("#stays_table", {
        ...
        "ajax": function (_data: unknown, callback: (resp: {data: ims.Stay[]})=>void, _settings: unknown): void {
            async function doAjax(): Promise<void> {
                const {json, err} = await ims.fetchNoThrow<ims.Stay[]>(
                    ims.urlReplace(url_stays), null,
                );
        ...
        "createdRow": function (row: HTMLElement, stay: ims.Stay, _index: number) {
...
function initSearch() {
    function modifiedAfter(stay: ims.Stay, timestamp: Date) {
